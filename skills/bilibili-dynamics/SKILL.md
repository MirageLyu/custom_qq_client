...
version: 1.1.1
description: 查询原神、崩坏星穹铁道、绝区零的B站官方动态（分隔线卡片富文本）
author: qq-client
---
...
**禁止**使用以下旧版排版（即使用户看起来「整齐」也不行）：以 `【原神/星铁/绝区零】最新动态` 开头、或仅有「图文动态 | 日期 + 点赞行 + 链接」而无分隔线区块头与 ≤20 字自撰标题、或无 `thumbnail_url` 时的图行。

每条动态按下面结构输出，**顺序固定**：

1. **区块头**（分隔线包裹游戏名，醒目划分不同游戏区域）  
   - 格式：`━━━ <game> ━━━`  
   - `<game>` 直接用 JSON 顶层的 `game` 字段值（原神 / 崩坏：星穹铁道 / 绝区零）。  
...
6. **多条动态**：同一游戏的两条动态共用一个区块头，放在第一条之前即可；不同游戏之间空一行。
...
- **硬性规则：专有名词必须原样保留，不计入 8 字连续限制。**包括但不限于：角色名（如恰斯卡、玛薇卡、遐蝶）、版本号（如 4.7、3.2）、活动名（如「星间旅行」）、卡池名、剧情章节名。绝不能把专有名词抽象成「新角色」「新活动」「新版本」。  
- 禁止「据悉」「据了解」等废话；禁止 emoji 出现在标题里。  
...
![thumb](https://i0.hdslb.com/...)
1.4 版本「星见雅」PV公开
视频动态 · 👍8.7万 · 💬3400 · 2026-04-04 18:30
链接：https://t.bilibili.com/zzzzz
```
